...
func TestExportGitSingleWorkspace(t *testing.T) {
	...
	if !strings.Contains(body, "Jmp-Snapshot: ") {
		t.Fatalf("expected Jmp-Snapshot trailer in commit body: %s", body)
	}
	if !strings.Contains(body, "Jmp-Workspace: ws-one") {
		t.Fatalf("expected Jmp-Workspace trailer in commit body: %s", body)
	}
	if !strings.Contains(body, "Jmp-Task: task-export-task-") {
		t.Fatalf("expected Jmp-Task trailer in commit body: %s", body)
	}

	// Verify mapping exists with 2 entries
	mapping, err := gitstore.LoadGitMapping(filepath.Join(projectRoot, ".jmp"))
	if err != nil {
...
func TestExportGitIncremental(t *testing.T) {
	...
	mapping1, err := gitstore.LoadGitMapping(filepath.Join(projectRoot, ".jmp"))
	if err != nil {
		t.Fatalf("LoadGitMapping: %v", err)
	}
	...
	mapping2, err := gitstore.LoadGitMapping(filepath.Join(projectRoot, ".jmp"))
	if err != nil {
		t.Fatalf("LoadGitMapping: %v", err)
	}
...
func TestCommitMetaFromSnapshot(t *testing.T) {
	...
	if meta2.AuthorEmail != "claude-code@jmp.local" {
