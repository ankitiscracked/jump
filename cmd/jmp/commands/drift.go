...
Examples:
  jmp drift                    # Drift vs main workspace
  jmp drift feature-branch     # Drift vs workspace named "feature-branch"
  jmp drift --no-dirty         # Compare committed snapshots only
  jmp drift --json             # Output as JSON
  jmp drift --agent-summary    # Generate AI risk assessment`,
