...
func TestProjectCreateCreatesMainWorkspace(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(projectPath, ".jmp", "config.json")); err != nil {
		t.Fatalf("expected project .jmp/config.json: %v", err)
	}
	if _, err := os.Stat(filepath.Join(workspacePath, ".jmp", "config.json")); err != nil {
