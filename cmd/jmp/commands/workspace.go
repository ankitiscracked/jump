...
Examples:
  jmp workspace create feature-1             # Fork from current/main workspace
  jmp workspace create bugfix --from dev     # Fork from 'dev' workspace`,
...
The main workspace is used as the default comparison target for 'jmp drift'.
...
Examples:
  jmp workspace set-main          # Set current workspace as main
  jmp workspace set-main dev      # Set workspace "dev" as main`,
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			var workspaceName string
			if len(args) > 0 {
...
func runSetMain(workspaceName string) error {
	...
	if workspaceName == "" {
		...
		if err != nil {
			return fmt.Errorf("workspace '%s' not found locally\nRun 'jmp info workspaces' to see available workspaces.", workspaceName)
		}
	...
	fmt.Println()
	fmt.Println("Other workspaces can now use 'jmp drift' to compare against this workspace.")
