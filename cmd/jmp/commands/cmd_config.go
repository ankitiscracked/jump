...
Examples:
  jmp config                              # interactive form (project-level)
  jmp config --global                     # interactive form (global)
  jmp config set name "John Doe"         # set project-level name
  jmp config set email "john@example.com" # set project-level email
  jmp config set --global name "John Doe" # set global name
  jmp config get                          # show resolved author
  jmp config get name                     # show specific field`,
		Args: cobra.NoArgs,
	...
	cmd.Flags().BoolVar(&global, "global", false, "Set globally (~/.config/jmp/)")
...
Examples:
  jmp config set name "John Doe"
  jmp config set email "john@example.com"
  jmp config set --global name "John Doe"`,
...
Examples:
  jmp config get          # show all
  jmp config get name     # show name only`,
...
func runConfigShow() error {
	...
	if author.IsEmpty() {
		fmt.Println("No author configured. Run 'jmp config' to set up.")
		return nil
	}
	fmt.Printf("  Name:  %s\n", author.Name)
	fmt.Printf("  Email: %s\n", author.Email)
