...
func TestSnapshotCreatesLocalArtifacts(t *testing.T) {
	...
	manifestsDir := filepath.Join(root, ".jmp", "manifests")
	snapshotsDir := filepath.Join(root, ".jmp", "snapshots")
	...
	blobDir := filepath.Join(root, ".jmp", "blobs")
...
func snapshotMetaPath(root, snapshotID string) string {
	return filepath.Join(root, ".jmp", "snapshots", snapshotID+".meta.json")
}
